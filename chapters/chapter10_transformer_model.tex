...
BERT-base uses $d_{\text{model}} = 768$, $h = 12$ ($d_k = 64$), $d_{ff} = 3072$, with $B = 32$ and $n = 512$.
...
BERT-base stacks $N = 12$ encoder layers with vocabulary $V = 30{,}000$, maximum sequence length 512, and learned position embeddings. The complete model has $\approx$110M parameters, of which embeddings account for $\sim$21\% and the transformer layers for $\sim$78\%. Training with batch size 32 and sequence length 512 requires $\approx$13.8~GB of GPU memory, dominated by activation storage ($\sim$87\%). See Section~\ref{sec:bert_base_analysis} for the full breakdown of parameters, activations, FLOPs, and hardware timing.
\end{example}
