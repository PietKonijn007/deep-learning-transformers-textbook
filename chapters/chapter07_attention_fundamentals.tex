...
\begin{keypoint}
The attention matrix requires $b \times h \times n^2 \times 4$ bytes of memory (batch size $\times$ heads $\times$ sequence length squared $\times$ FP32). For BERT-base ($b=32$, $h=12$, $n=512$), this is $\sim$403~MB per layer, or $\sim$4.8~GB across 12 layers. This quadratic memory cost is the primary bottleneck for long sequences and motivates efficient attention methods (Chapters~9 and~16).
\end{keypoint}
