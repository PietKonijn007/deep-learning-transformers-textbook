...
\begin{keypoint}
The generic framework for drift detection (performance monitoring, distribution monitoring, confidence tracking), continuous learning strategies (periodic retraining, triggered retraining, online learning, ensemble approaches, human-in-the-loop), and practical implementation infrastructure (data versioning, model versioning, automated pipelines, monitoring dashboards) is covered in Chapter~\ref{chap:domainspecificmodels}, Section~\ref{sec:continuouslearning}. Here we highlight NLP-specific considerations.
\end{keypoint}

Key NLP-specific adaptation strategies include:
\begin{itemize}
\item \textbf{Vocabulary updates:} New products and terminology require updating the tokenizer and model vocabulary. Add tokens for new domain terms that would otherwise fragment into meaningless subwords.
\item \textbf{Active learning for efficient labeling:} Use model uncertainty to prioritize which recent examples to label, focusing annotation budget on drifting areas (e.g.\ new product categories).
\item \textbf{Vocabulary monitoring:} Track unknown-token frequency and subword fragmentation rates---spikes indicate terminology drift before performance degrades.
\end{itemize}

\subsection{Case Study: Support Ticket Classification Drift}
