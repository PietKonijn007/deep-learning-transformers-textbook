...
\begin{tikzpicture}[
  ...
  layer/.style={rectangle, draw, minimum width=3cm, minimum height=0.8cm, font=\small},
  arrow/.style={->, thick},
...
\draw[bidir] (t2) -- (t3);
\draw[bidir] (t3) -- (t4);
\draw[bidir] (t1) to[bend left=20] (t3);
\draw[bidir] (t2) to[bend left=20] (t4);
\draw[bidir] (t1) to[bend left=30] (t4);

% Output layer
\node[layer, fill=green!10] (enc) at (3,3) {BERT Encoder (12 layers)};
\draw[arrow] (t1) -- (enc);
\draw[arrow] (t2) -- (enc);
\draw[arrow] (t3) -- (enc);
\draw[arrow] (t4) -- (enc);
...
\end{tikzpicture}
\caption{BERT's bidirectional encoder architecture. Blue double-headed arrows show that each token can attend to all other tokens in both directions, enabling rich contextual representations. Each output $h_i$ depends on the entire input sequence.}
\label{fig:bert_bidirectional_attention}
\end{figure}
