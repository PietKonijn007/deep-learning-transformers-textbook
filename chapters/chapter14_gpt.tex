...
% Causal attention connections (only backward)
\draw[causal] (t1) -- (t2);
\draw[causal] (t2) -- (t3);
\draw[causal] (t3) -- (t4);
\draw[causal] (t1) to[bend right=20] (t3);
\draw[causal] (t2) to[bend right=20] (t4);
\draw[causal] (t1) to[bend right=30] (t4);

% Decoder layer
\node[layer, fill=orange!10] (dec) at (3,3) {GPT Decoder (12-96 layers)};
\draw[arrow] (t1) -- (dec);
\draw[arrow] (t2) -- (dec);
\draw[arrow] (t3) -- (dec);
\draw[arrow] (t4) -- (dec);
...
$h_1$ sees: $x_1$ \\
$h_2$ sees: $x_1, x_2$ \\
$h_3$ sees: $x_1, x_2, x_3$ \\
$h_4$ sees: $x_1, x_2, x_3, x_4$
};

\end{tikzpicture}
\caption{GPT's causal decoder architecture. Red arrows show unidirectional attention where each token can only attend to previous tokens (including itself). This triangular connectivity pattern enables autoregressive generation while preventing information leakage from future positions.}
\label{fig:gpt_causal_attention}
\end{figure}
