\chapter{Autonomous Systems and Observability}
...
\section{AIOps: Intelligent IT Operations}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{chapters/diagrams/chapter15_aiops_architecture_a1b2c3d4.pdf}
\caption{AIOps multi-modal architecture combining log language models, time series transformers, and graph neural networks for automated incident detection and root cause analysis}
\label{fig:aiops_architecture}
\end{figure}

\section{Agent-Based Automation}
...
\subsection{Performance and Economics}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{chapters/diagrams/chapter15_agent_workflow_e5f6g7h8.pdf}
\caption{Agent-based automation workflow showing task decomposition, tool selection, and approval gates for safe autonomous operation}
\label{fig:agent_workflow}
\end{figure}

\section{Security Threats Unique to Autonomous Systems}
...
\subsection{Threats Unique to Autonomous Systems}
...
\subsubsection{Instruction-Level Attacks: Agentic AI Vulnerabilities}
...
\subsubsection{Layer 1: Input Validation and Sanitization (Foundation)}
...
\subsubsection{Layer 2: Instruction Isolation (Critical)}

Instruction isolation separates system instructions from user inputs, preventing user input from overriding system directives. XML-tagged instructions mark system guidelines with special markers, preventing semantic confusion with user input. A well-structured prompt wraps system instructions in \texttt{<SYSTEM\_INSTRUCTIONS>} tags and explicitly states that the model must follow these instructions exactly and not allow \texttt{<USER\_INPUT>} to override them.
...
\subsubsection{Layer 3: Retrieval Augmentation Security (Emerging - Rapidly Evolving)}
...
\subsubsection{Layer 4: Tool Execution Controls (Most Mature)}

Tool execution controls prevent agents from taking unintended actions even if instructions are compromised. This is the most mature defensive layer. The principle of least privilege ensures tools have minimum required permissions—a database analysis tool can execute SELECT queries but not UPDATE operations. Approval gates classify actions by risk: read-only operations (logs, metrics, status checks) execute automatically, low-risk modifications (single service restart, cache clear) execute after baseline reliability is proven, medium-risk operations (resource scaling, configuration changes) require human approval, and high-risk operations (database schema changes, multi-service restarts) are always prohibited or require elevated approval.
...
\subsubsection{Layer 5: Watermarking and Attribution (Rapidly Evolving)}
...
\subsubsection{Layer 6: Privacy-Preserving Learning (Proven at Scale)}
...
\subsubsection{Layer 7: Behavioral Monitoring and Anomaly Detection (Rapidly Improving)}
...
\subsubsection{Layer 8: Model Risk Management Governance (Established Framework)}
...
\subsection{Which Areas Are Evolving Fastest}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{chapters/diagrams/chapter15_security_defense_layers_c5d6e7f8.pdf}
\caption{Defense-in-depth security architecture showing eight complementary layers protecting autonomous systems against instruction-level, data-level, and execution-level attacks}
\label{fig:security_defense_layers}
\end{figure}
...
\subsection{Organizational Integration}
