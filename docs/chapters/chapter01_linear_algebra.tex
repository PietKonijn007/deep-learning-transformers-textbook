...
Understanding the full computational profile of a transformer layer---parameters, FLOPs, memory, and hardware utilization---is essential for practical deep learning engineering. Section~\ref{sec:bert_base_analysis} provides a comprehensive, self-contained worked analysis of BERT-base, the standard baseline configuration used as a running example throughout this textbook.
...
\section{BERT-base: A Canonical Worked Analysis}
\label{sec:bert_base_analysis}
...
\textbf{Per encoder layer.} Each layer has three components:
\begin{align}
...
\textbf{Total per layer:} \quad &\mathbf{7{,}084{,}800} \text{ parameters}
\end{align}
...
\textbf{Total:} \quad &\mathbf{109{,}044{,}480 \approx 110\text{M parameters}}
\end{align}
...
\subsection{Dimension Tracking Through One Layer}
\label{subsec:bert_dimension_tracking}

\textbf{Input:} $\mX \in \R^{32 \times 512 \times 768}$ (batch $\times$ sequence $\times$ model dimension)
...
\begin{tabular}{lr}
\toprule
\textbf{Component (per layer)} & \textbf{Memory (FP32)} \\
\midrule
...
\midrule
\textbf{Per-layer total} & \textbf{704 MB} \\
\textbf{12 layers} & \textbf{8.4 GB} \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Self-Attention (per layer):}
\begin{align}
...
\textbf{Feed-Forward Network (per layer):}
\begin{align}
...
\midrule
\textbf{Total} & $\approx$\textbf{13.8 GB} \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{Hardware Timing (NVIDIA A100)}
...
\textbf{Memory bandwidth check:} Loading 110M parameters ($\times$ 4 bytes $= 440$~MB) takes $440/1600 \approx 0.28$~ms, comparable to compute time. For small batch sizes, BERT-base becomes memory-bandwidth bound rather than compute-bound; larger batches amortize parameter loading across more computation.
...
\begin{solution}[Exercise~\ref{ex:ch1_ex1}]
Given $\vx = [2, -1, 3]\transpose$ and $\vy = [1, 4, -2]\transpose$:
...
\begin{solution}[Exercise~\ref{ex:ch1_ex2}]
For transformer layer with $d_{\text{model}} = 768$ and $d_{ff} = 3072$:

\textbf{(1) Parameters in feed-forward network:}
\begin{itemize}
    \item First linear: $\mW_1 \in \R^{3072 \times 768}$ has $3072 \times 768 = 2{,}359{,}296$ weights, plus bias $\vb_1 \in \R^{3072}$ has $3{,}072$ parameters
    \item Second linear: $\mW_2 \in \R^{768 \times 3072}$ has $768 \times 3072 = 2{,}359{,}296$ weights, plus bias $\vb_2 \in \R^{768}$ has $768$ parameters
    \item Total: $2{,}359{,}296 + 3{,}072 + 2{,}359{,}296 + 768 = 4{,}722{,}432$ parameters
\end{itemize}

\textbf{(2) Input tensor dimensions:}
...
\begin{itemize}
    \item First transformation: $2 \times (Bn) \times d_{\text{model}} \times d_{ff} = 2 \times (32 \times 512) \times 768 \times 3072 = 77{,}309{,}411{,}328$ FLOPs
    \item Second transformation: $2 \times (Bn) \times d_{ff} \times d_{\text{model}} = 77{,}309{,}411{,}328$ FLOPs
    \item Total: $154{,}618{,}822{,}656 \approx 154.6$ GFLOPs
\end{itemize}
\end{solution}

\begin{solution}[Exercise~\ref{ex:ch1_ex3}]
\textbf{Proof:} Let $\mA = \mA\transpose$ be symmetric with eigenvectors $\vv_1, \vv_2$ corresponding to distinct eigenvalues $\lambda_1 \neq \lambda_2$.
...
\begin{solution}[Exercise~\ref{ex:ch1_ex4}]
For weight matrix $\mW \in \R^{1024 \times 4096}$ with rank-$r$ SVD approximation:

\textbf{(1) Parameters as function of $r$:}
The factored form requires:
\begin{itemize}
    \item $\mW_1 = \mU_r \boldsymbol{\Sigma}_r \in \R^{1024 \times r}$: $1024r$ parameters
    \item $\mW_2 = \mV_r\transpose \in \R^{r \times 4096}$: $4096r$ parameters
    \item Total: $1024r + 4096r = 5120r$ parameters
\end{itemize}

\textbf{(2) Value of $r$ for 75\% compression:}
...
\textbf{(3) Memory savings:}
\begin{itemize}
    \item Original: $4{,}194{,}304 \times 4 \text{ bytes} = 16{,}777{,}216 \text{ bytes} \approx 16.0$ MB
    \item Compressed: $1{,}048{,}576 \times 4 \text{ bytes} = 4{,}194{,}304 \text{ bytes} \approx 4.0$ MB
    \item Savings: $16.0 - 4.0 = 12.0$ MB
\end{itemize}
\end{solution}

\begin{solution}[Exercise~\ref{ex:ch1_ex5}]
For attention scores $\mA = \mQ\mK\transpose$ with $\mQ, \mK \in \R^{B \times n \times d_k}$, $B = 16$, $n = 1024$, $d_k = 64$:
...
\textbf{(3) Arithmetic intensity:}
Memory transferred:
\begin{itemize}
    \item Read $\mQ$: $16 \times 1024 \times 64 \times 4 = 4{,}194{,}304$ bytes
    \item Read $\mK$: $16 \times 1024 \times 64 \times 4 = 4{,}194{,}304$ bytes
    \item Write $\mA$: $16 \times 1024 \times 1024 \times 4 = 67{,}108{,}864$ bytes
    \item Total: $75{,}497{,}472$ bytes $\approx 72$ MB
\end{itemize}
...
\textbf{(4) Compute-bound or memory-bound:}
\begin{itemize}
    \item Compute time: $\frac{2.15 \text{ GFLOPs}}{312 \text{ TFLOPs}} \approx 6.9$ microseconds
    \item Memory time: $\frac{72 \text{ MB}}{1{,}600{,}000 \text{ MB/s}} \approx 45$ microseconds
    \item The operation is \textbf{memory-bound} by a factor of $45/6.9 \approx 6.5\times$
\end{itemize}
\end{solution}

\begin{solution}[Exercise~\ref{ex:ch1_ex6}]
For embedding layer with $V = 32{,}000$ and $d = 512$:

\textbf{(1) Number of parameters:}
\begin{equation}
V \times d = 32{,}000 \times 512 = 16{,}384{,}000 \text{ parameters}
\end{equation}

\textbf{(2) Memory requirement:}
\begin{equation}
16{,}384{,}000 \times 4 \text{ bytes} = 65{,}536{,}000 \text{ bytes} \approx 62.5 \text{ MB}
\end{equation}

\textbf{(3) Memory for embedded representations:}
...
\textbf{(4) LoRA trainable parameters:}
...
\begin{solution}[Exercise~\ref{ex:ch1_ex7}]
For $\mA \in \R^{512 \times 2048}$, $\mB \in \R^{2048 \times 512}$, $\vx \in \R^{512}$:

\textbf{Option 1: $(\mA\mB)\vx$}
\begin{itemize}
    \item Compute $\mC = \mA\mB \in \R^{512 \times 512}$: $2 \times 512 \times 2048 \times 512 = 1{,}073{,}741{,}824$ FLOPs
    \item Compute $\mC\vx \in \R^{512}$: $2 \times 512 \times 512 = 524{,}288$ FLOPs
    \item Total: $1{,}074{,}266{,}112$ FLOPs
\end{itemize}

\textbf{Option 2: $\mA(\mB\vx)$}
\begin{itemize}
    \item Compute $\vy = \mB\vx \in \R^{2048}$: $2 \times 2048 \times 512 = 2{,}097{,}152$ FLOPs
    \item Compute $\mA\vy \in \R^{512}$: $2 \times 512 \times 2048 = 2{,}097{,}152$ FLOPs
    \item Total: $4{,}194{,}304$ FLOPs
\end{itemize}
...
\begin{solution}[Exercise~\ref{ex:ch1_ex8}]
For $\mC = \mA\mB$ with $\mA, \mB \in \R^{2048 \times 2048}$:
...
\textbf{(2) Memory transferred:}
\begin{itemize}
    \item Read $\mA$: $2048^2 \times 4 = 16{,}777{,}216$ bytes
    \item Read $\mB$: $2048^2 \times 4 = 16{,}777{,}216$ bytes
    \item Write $\mC$: $2048^2 \times 4 = 16{,}777{,}216$ bytes
    \item Total: $50{,}331{,}648$ bytes $\approx 48$ MB
\end{itemize}
...
\textbf{(4) Theoretical execution time:}
\begin{itemize}
    \item Compute time: $\frac{17.2 \text{ GFLOPs}}{100 \text{ TFLOPs}} = 0.172$ ms
    \item Memory time: $\frac{48 \text{ MB}}{900{,}000 \text{ MB/s}} = 0.053$ ms
    \item Execution time: $\max(0.172, 0.053) = 0.172$ ms
\end{itemize}

\textbf{(5) Bottleneck:}
The operation is \textbf{compute-bound} since compute time (0.172 ms) exceeds memory time (0.053 ms). The high arithmetic intensity (341 FLOP/byte) makes this operation well-suited for GPU acceleration.
\end{solution}
