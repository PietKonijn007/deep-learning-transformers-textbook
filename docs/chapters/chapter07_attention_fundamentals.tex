...
Dot-product & $\vq\transpose\vk$ & 0 & 80--90\% & Not used (unstable) \\
Scaled dot-product & $\vq\transpose\vk/\sqrt{d_k}$ & 0 & 80--90\% & All transformers \\
...
\begin{keypoint}
The attention matrix requires $b \times h \times n^2 \times 4$ bytes of memory (batch size $\times$ heads $\times$ sequence length squared $\times$ FP32). For BERT-base ($b=32$, $h=12$, $n=512$), this is $\sim$402~MB per layer, accounting for 57\% of per-layer activation memory (see Section~\ref{sec:bert_base_analysis} for the complete memory breakdown). This quadratic cost is the primary bottleneck for long sequences and motivates efficient attention methods (Chapters~9 and~16).
\end{keypoint}
