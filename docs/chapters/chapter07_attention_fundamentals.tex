...
\textbf{Parallelization Benefit:} The crucial advantage is that attention enables parallelization. Unlike RNN hidden states that must be computed sequentially, attention weights for all output positions can be computed simultaneously during training when the target sequence is known. This transforms the sequential $O(m)$ decoder steps into a single parallel operation, dramatically improving GPU utilization from 5-10\% to 60-80\% in practice.

\begin{example}[Translation with Attention]
\label{ex:translation_attention}
Consider translating the English sentence "The cat sat on the mat" to French: "Le chat était assis sur le tapis." Without attention, the encoder compresses all six English words into a single 512-dimensional context vector, which the decoder uses to generate all seven French words. The context vector must simultaneously encode that "cat" translates to "chat," "sat" translates to "était assis," and "mat" translates to "tapis"—a challenging compression task.
...
\textbf{Computational Cost Analysis:} The additive scoring function in Step 1 requires substantial computation for each query-key pair. For a single alignment score $e_{t,i}$, we must:
\begin{enumerate}
    \item Compute $\mW_1 \mathbf{s}_t$: $2d_a d_s$ FLOPs (matrix-vector multiplication)
    \item Compute $\mW_2 \vh_i$: $2d_a d_h$ FLOPs
    \item Add the results: $d_a$ FLOPs
    \item Apply $\tanh$: $\approx 3d_a$ FLOPs (exponentials and divisions)
    \item Compute $\mathbf{v}\transpose (\cdot)$: $2d_a$ FLOPs
\end{enumerate}
...
\textbf{Memory Requirements:} The attention mechanism requires storing:
\begin{itemize}
    \item Encoder hidden states: $n \times d_h$ elements
    \item Alignment scores: $m \times n$ elements (for all decoder positions during training)
    \item Attention weights: $m \times n$ elements
    \item Intermediate activations: $m \times n \times d_a$ elements for the $\tanh$ layer
\end{itemize}
...
\textbf{Parameter Count:} Bahdanau attention introduces $O(d_a(d_s + d_h))$ parameters:
\begin{align}
...
\textbf{Step 1:} Compute alignment scores for each encoder position. Suppose after applying $\mW_1 \mathbf{s}_2 + \mW_2 \vh_i$ and passing through $\tanh$ and $\mathbf{v}\transpose$, we obtain:
\begin{align}
e_{2,1} &= 0.8 \\
e_{2,2} &= 2.1 \\
e_{2,3} &= 0.5
...
\textbf{Step 2:} Apply softmax to convert scores to a probability distribution:
\begin{align}
...
\textbf{Step 3:} Compute the context vector as a weighted sum:
\begin{equation}
...
\centering
\begin{tikzpicture}[
    matrix/.style={rectangle, draw, fill=blue!15, minimum width=1.5cm, minimum height=1cm, font=\small},
    operation/.style={rectangle, draw, fill=green!15, minimum width=1.8cm, minimum height=0.7cm, font=\small},
...
\textbf{Softmax Saturation Problem:} The softmax function is defined as:
\begin{equation}
...
\textbf{Gradient Flow Analysis:} The gradient of softmax with respect to its input is:
\begin{equation}
...
\textbf{Scaling Solution:} Dividing by $\sqrt{d_k}$ normalizes the variance:
\begin{equation}
...
\textbf{FLOP Count Comparison:} For $m$ queries and $n$ keys with dimension $d_k$:
...
\textit{Bahdanau attention:} As computed earlier, for $d_a = 256$, $d_s = d_h = 512$, $m = n = 512$:
\begin{equation}
...
\textbf{Memory Bandwidth Considerations:} Modern GPUs are often memory-bandwidth limited rather than compute-limited. The NVIDIA A100 has 312 TFLOPS of FP16 compute but only 1.5 TB/s memory bandwidth. For operations to be compute-bound, they must perform many FLOPs per byte loaded from memory.
...
\textbf{Practical Performance:} On an NVIDIA A100 GPU, computing attention for a batch of 32 sequences with $n = 512$ and $d_k = 64$:
\begin{itemize}
    \item Scaled dot-product attention: $\approx 0.8$ ms (achieving 250 TFLOPS, 80\% of peak)
    \item Bahdanau attention: $\approx 15$ ms (achieving 15 TFLOPS, 5\% of peak)
\end{itemize}
...
\textbf{Step 1:} Compute dot products between the query and each key:
\begin{align}
...
\textbf{Step 3:} Apply softmax to obtain attention weights:
\begin{align}
...
\textbf{Step 4:} Apply attention weights to values. Suppose:
\begin{equation}
\mV = \begin{bmatrix}
0.5 & 0.8 & -0.2 & 0.6 & 0.3 \\
0.2 & -0.4 & 0.7 & 0.1 & 0.9 \\
...
\section{Attention Score Computation Methods}
\label{sec:attention_methods_comparison}
...
\subsection{Taxonomy of Attention Mechanisms}
...
\textbf{Additive Attention (Bahdanau):} Introduced in 2015 for neural machine translation, additive attention computes scores through a learned feedforward network:
\begin{equation}
...
For typical dimensions $d_q = d_k = 512$ and $d_a = 256$, computing one score requires approximately $2 \times 256 \times (512 + 512) = 524{,}288$ FLOPs. For a sequence of length 512 attending to itself, this amounts to $512^2 \times 524{,}288 \approx 137$ billion FLOPs just for score computation. The $\tanh$ nonlinearity and multiple matrix-vector products prevent efficient GPU utilization, achieving only 15-25\% of peak performance.

\textbf{Dot-Product Attention:} The simplest scoring function is the unscaled dot product:
\begin{equation}
\text{score}(\vq, \vk) = \vq\transpose \vk
\end{equation}

This requires only $2d_k$ FLOPs (one multiply-add per dimension) and introduces zero parameters. The computation maps perfectly to GPU hardware through matrix multiplication: computing all scores $\mQ \mK\transpose$ is a single GEMM operation achieving 80-90\% of peak performance.
...
\textbf{Scaled Dot-Product Attention:} Adding the scaling factor $1/\sqrt{d_k}$ solves the variance problem while preserving computational efficiency:
\begin{equation}
...
\textbf{General Attention (Luong):} Luong attention introduces a learned transformation matrix:
\begin{equation}
\text{score}(\vq, \vk) = \vq\transpose \mW \vk
\end{equation}

where $\mW \in \R^{d_q \times d_k}$ is a learned parameter matrix. This allows the model to learn a task-specific similarity metric rather than using raw dot products. The transformation can project queries and keys into a shared space where their dot product is more meaningful.

Computing one score requires $2d_q d_k$ FLOPs (matrix-vector product $\mW \vk$, then dot product with $\vq$), and the mechanism introduces $d_q d_k$ parameters. For $d_q = d_k = 512$, this is 262,144 parameters—substantial but manageable. The computational cost is $2 \times 512 \times 512 = 524{,}288$ FLOPs per score, similar to additive attention but without the nonlinearity.

General attention achieves better GPU utilization than additive attention because the core operation $\mQ \mW \mK\transpose$ can be computed as two matrix multiplications. However, it still requires more computation than scaled dot-product attention and introduces parameters that must be learned.
...
\begin{tabular}{lcccc}
\hline
\textbf{Method} & \textbf{Computation} & \textbf{Parameters} & \textbf{GPU Util.} & \textbf{Used In} \\
\hline
Additive (Bahdanau) & $\mathbf{v}\transpose\tanh(\mW_1\vq + \mW_2\vk)$ & $O(d_a(d_q + d_k))$ & 15-25\% & Early seq2seq \\
Dot-product & $\vq\transpose\vk$ & 0 & 80-90\% & Not used \\
Scaled dot-product & $\vq\transpose\vk/\sqrt{d_k}$ & 0 & 80-90\% & Transformers \\
General (Luong) & $\vq\transpose\mW\vk$ & $O(d_q d_k)$ & 50-70\% & Some seq2seq \\
\hline
\end{tabular}
\end{center}

\textbf{Why Scaled Dot-Product Won:} The dominance of scaled dot-product attention in modern transformers reflects several factors beyond raw computational efficiency. First, the parameter-free nature means the model's capacity is concentrated in the QKV projection matrices and feed-forward networks rather than the scoring function. This architectural choice scales better to very large models—GPT-3 with 175 billion parameters would require even more parameters if using additive or general attention.

Second, the simplicity of scaled dot-product attention makes it easier to optimize and implement efficiently. Hardware vendors can optimize specifically for the $\mQ \mK\transpose$ operation, and software frameworks can apply specialized kernels. FlashAttention and other efficient attention implementations focus on scaled dot-product attention because its regular structure enables aggressive optimization.
...
\textbf{Memory Bandwidth Analysis:} Modern GPUs are often memory-bandwidth limited, meaning performance depends on how efficiently data is moved between memory and compute units. Scaled dot-product attention achieves high arithmetic intensity—many FLOPs per byte loaded—because the matrix multiplication $\mQ \mK\transpose$ reuses each element of $\mQ$ and $\mK$ multiple times.

For $\mQ, \mK \in \R^{512 \times 64}$ in FP16, we load $2 \times 512 \times 64 \times 2 = 131$ KB and perform $2 \times 512^2 \times 64 = 33.5$ million FLOPs, achieving $33{,}500{,}000 / 131{,}072 \approx 256$ FLOPs per byte. This high intensity keeps compute units busy rather than waiting for memory.
...
\textbf{Concrete Example:} In machine translation, when generating the French word "chat" (cat) from the English sentence "The cat sat on the mat," the decoder's query encodes "I need information about the subject noun." The keys encode what each English word represents: "the" offers determiner information, "cat" offers subject noun information, "sat" offers verb information, etc. The attention mechanism computes high similarity between the query and the key for "cat," then retrieves the value associated with "cat"—a rich representation encoding its meaning, grammatical role, and context.
...
\textbf{Computational Cost:} Each projection is a matrix multiplication requiring $2nd_{\text{model}}d_k$ FLOPs (for queries and keys) or $2nd_{\text{model}}d_v$ FLOPs (for values). With three projections and $d_k = d_v$:
\begin{equation}
...
\textbf{Parameter Count:} The three projection matrices introduce $d_{\text{model}}(2d_k + d_v)$ parameters per attention head. For $h$ heads with $d_k = d_v = d_{\text{model}}/h$:
\begin{equation}
...
\textbf{Attention computation:} After projection, we compute attention:
\begin{equation}
...
\section{Hardware Implications of Attention}
\label{sec:hardware_implications}
...
\subsection{Parallelization: RNNs vs Attention}

Recurrent neural networks process sequences sequentially by design. Each hidden state $\vh_t$ depends on the previous hidden state $\vh_{t-1}$:
\begin{equation}
\vh_t = \text{RNN}(\vx_t, \vh_{t-1})
\end{equation}

This recurrence creates a data dependency chain: we cannot compute $\vh_t$ until $\vh_{t-1}$ is available, which requires $\vh_{t-2}$, and so on back to $\vh_0$. For a sequence of length $n$, we must perform $n$ sequential operations, even if we have thousands of parallel compute units available.
...
\begin{equation}
\text{output}_i = \sum_{j=1}^{n} \alpha_{ij} \vv_j
\end{equation}

All attention weights $\alpha_{ij}$ can be computed simultaneously because they depend only on the queries and keys, which are computed from the input via matrix multiplication. During training, when the entire target sequence is known, we can compute attention for all positions in parallel. For 32 sequences of length 512, we can utilize all 16,384 positions simultaneously, achieving near-full GPU utilization.

\textbf{Practical Impact:} On an NVIDIA A100 GPU, processing a batch of 32 sequences of length 512 with hidden dimension 768:
\begin{itemize}
    \item LSTM encoder: $\approx 45$ ms (sequential processing, 5\% GPU utilization)
    \item Transformer encoder: $\approx 3$ ms (parallel processing, 75\% GPU utilization)
\end{itemize}

The 15× speedup from parallelization is what makes training large transformers feasible. GPT-3 with 175 billion parameters was trained on 300 billion tokens—a task that would be impractical with sequential RNN processing.
...
Attention's core operation $\mQ \mK\transpose$ is a matrix multiplication, which achieves high arithmetic intensity through data reuse. For $\mQ, \mK \in \R^{512 \times 64}$:
\begin{itemize}
    \item Data loaded: $2 \times 512 \times 64 \times 2 = 131$ KB (FP16)
    \item FLOPs performed: $2 \times 512^2 \times 64 = 33.5$ million
    \item Arithmetic intensity: $33{,}500{,}000 / 131{,}072 \approx 256$ FLOPs/byte
\end{itemize}
...
\textbf{Memory Requirements:} Attention does require more memory than RNNs due to storing the attention matrix. For a sequence of length $n$ with batch size $b$ and $h$ heads:
\begin{equation}
\text{Attention matrix memory} = b \times h \times n^2 \times 4 \text{ bytes (FP32)}
\end{equation}

For BERT-base with $b = 32$, $h = 12$, $n = 512$:
\begin{equation}
32 \times 12 \times 512^2 \times 4 = 402{,}653{,}184 \text{ bytes} \approx 403 \text{ MB}
\end{equation}
...
Consider computing $\mQ \mK\transpose$ for a single sequence ($b = 1$) versus a batch ($b = 32$):
\begin{itemize}
    \item Single sequence: $\mQ, \mK \in \R^{512 \times 64}$, output $\in \R^{512 \times 512}$
    \begin{itemize}
        \item FLOPs: $2 \times 512^2 \times 64 = 33.5$ million
        \item Time on A100: $\approx 0.15$ ms (achieves 30\% peak, memory-bound)
    \end{itemize}
    
    \item Batch of 32: $\mQ, \mK \in \R^{16384 \times 64}$, output $\in \R^{16384 \times 16384}$
    \begin{itemize}
        \item FLOPs: $2 \times 16384^2 \times 64 = 34.4$ billion
        \item Time on A100: $\approx 120$ ms (achieves 85\% peak, compute-bound)
    \end{itemize}
\end{itemize}

The batched computation achieves $34.4 / 33.5 = 1024$ times more FLOPs in $120 / 0.15 = 800$ times more time, demonstrating the efficiency gain from batching. Per-sequence processing time drops from 0.15 ms to $120 / 32 = 3.75$ ms, but the throughput increases from $1 / 0.15 = 6.7$ sequences/ms to $32 / 120 = 0.27$ sequences/ms... wait, that's wrong. Let me recalculate: throughput increases from $1 / 0.15 \approx 6.7$ sequences/second to $32 / 120 \approx 267$ sequences/second—a 40× improvement in throughput.
...
For scaled dot-product attention with $\mQ \in \R^{10 \times 64}$, $\mK \in \R^{20 \times 64}$, $\mV \in \R^{20 \times 128}$:

\textbf{(1) Output dimension:}
\begin{equation}
\text{Output} = \text{softmax}\left(\frac{\mQ\mK\transpose}{\sqrt{d_k}}\right)\mV \in \R^{10 \times 128}
\end{equation}

\textbf{(2) Attention matrix shape:}
\begin{equation}
\mA = \mQ\mK\transpose \in \R^{10 \times 20}
\end{equation}

\textbf{(3) FLOPs for $\mQ\mK\transpose$:}
\begin{equation}
...
\textbf{Variance analysis without scaling:}
...
\textbf{Effect on softmax gradients:}

Without scaling, dot products range roughly $[-24, 24]$ (3 standard deviations). After softmax:
\begin{itemize}
    \item Large positive scores $\to$ probability $\approx 1$
    \item Large negative scores $\to$ probability $\approx 0$
    \item Softmax saturates, gradients vanish
\end{itemize}

\textbf{Numerical demonstration:}
\begin{align}
