...
\begin{keypoint}
For the generic drift detection and continuous learning framework, see Chapter~\ref{chap:domainspecificmodels}, Section~\ref{sec:continuouslearning}. Recommendation systems typically require the most aggressive retraining cadence (daily to weekly) due to the rapid pace of preference and content evolution.
\end{keypoint}

Key recommendation-specific strategies beyond the generic framework include:
\begin{itemize}
\item \textbf{Separate short-term and long-term models:} Use a transformer on recent interactions for ``what the user wants right now'' and a collaborative filter on full history for ``what the user generally likes,'' combining with learned weights.
\item \textbf{Contextual bandits for exploration:} Allocate 5--10\% of recommendations to exploration via Thompson sampling or UCB, enabling continuous learning about new items and shifting preferences.
\item \textbf{Seasonal and event-aware modeling:} Explicitly include temporal features (day-of-week, holiday indicators, event flags) and train separate model components for predictable seasonal shifts.
\item \textbf{Online user embedding updates:} Update user embeddings hourly based on recent interactions while keeping item embeddings on a daily cycle, balancing freshness with stability.
\end{itemize}
