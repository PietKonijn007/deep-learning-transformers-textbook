...
\section{Computational Complexity and Hardware Analysis}
...
For a single transformer encoder layer processing a sequence of length $n$ with model dimension $d_{\text{model}}$, feed-forward dimension $d_{ff}$, and $h$ attention heads (where $d_k = d_{\text{model}}/h$), the computational cost breaks down as follows. The multi-head attention requires four matrix multiplications for Q, K, V, and output projections, each costing $2nd_{\text{model}}^2$ FLOPs (the factor of 2 accounts for both multiplication and addition in matrix multiplication). The attention score computation $\mQ \mK^T$ requires $2n^2d_{\text{model}}$ FLOPs, and the attention-weighted sum $\text{Attn} \mV$ requires another $2n^2d_{\text{model}}$ FLOPs. The feed-forward network requires $2nd_{\text{model}}d_{ff}$ FLOPs for the first projection and another $2nd_{\text{model}}d_{ff}$ FLOPs for the second projection.

\begin{definition}[Transformer Layer Computational Cost]
\label{def:transformer_flops}
...
\textbf{Feed-forward network (assuming $d_{ff} = 4d_{\text{model}}$):}
\begin{align}
...
\begin{example}[BERT-base Training FLOPs]
\label{ex:bert_flops}
...
\textbf{All 12 layers:} $12 \times 248.0 = 2{,}976$ GFLOPs $\approx 3.0$ TFLOPs per training step

\textbf{Backward pass:} Approximately $2 \times$ forward pass = $6.0$ TFLOPs

\textbf{Total per training step:} $3.0 + 6.0 = 9.0$ TFLOPs

\textbf{Training time on NVIDIA A100 (312 TFLOPS FP16):}
...
\textbf{Arithmetic intensity:} $77.3 \times 10^9 / (130.6 \times 10^6) = 592$ FLOPs/byte
...
\textbf{Arithmetic intensity:} $1.3 \times 10^9 / (50.3 \times 10^6) = 26$ FLOPs/byte
...
\begin{example}[GPT-3 Scale Analysis]
\label{ex:gpt3_scale}
...
\textbf{Total model:} $96 \times 1{,}812 = 173{,}952$M $\approx 174$B parameters (plus embeddings $\approx 1$B)
...
\subsection{Encoder-Only Architecture (BERT)}
...
\subsection{Decoder-Only Architecture (GPT)}
...
\subsection{Encoder-Decoder Architecture (T5, BART)}
...
\begin{example}[Architecture Comparison: BERT vs GPT-2 vs T5-base]
\label{ex:architecture_comparison}

\textbf{BERT-base (Encoder-only):}
\begin{itemize}
\item Layers: 12 encoder layers
\item Dimensions: $d_{\text{model}} = 768$, $h = 12$, $d_{ff} = 3072$
\item Parameters: 110M (embeddings + 12 encoder layers)
\item Attention: Bidirectional self-attention in all layers
\item Pre-training: Masked language modeling (predict masked tokens)
\item Inference: Single forward pass for sequence length $n$
\item Best for: Classification, NER, extractive QA
\end{itemize}

\textbf{GPT-2 (Decoder-only):}
\begin{itemize}
\item Layers: 12 decoder layers (but no cross-attention, so effectively simplified decoders)
\item Dimensions: $d_{\text{model}} = 768$, $h = 12$, $d_{ff} = 3072$
\item Parameters: 117M (embeddings + 12 decoder layers without cross-attention)
\item Attention: Causal self-attention in all layers
\item Pre-training: Autoregressive language modeling (predict next token)
\item Inference: $n$ forward passes for sequence length $n$ (autoregressive)
\item Best for: Text generation, few-shot learning, dialogue
\end{itemize}

\textbf{T5-base (Encoder-Decoder):}
\begin{itemize}
\item Layers: 12 encoder layers + 12 decoder layers
\item Dimensions: $d_{\text{model}} = 768$, $h = 12$, $d_{ff} = 3072$
\item Parameters: 220M (embeddings + 12 encoders + 12 decoders with cross-attention)
\item Attention: Bidirectional in encoder, causal + cross-attention in decoder
\item Pre-training: Span corruption (predict masked spans)
\item Inference: One encoder pass + $m$ decoder passes for output length $m$
\item Best for: Translation, summarization, generative QA
\end{itemize}

\textbf{Parameter breakdown comparison:}
\begin{align}
...
\textbf{Computational cost for sequence length 512:}
\begin{align}
