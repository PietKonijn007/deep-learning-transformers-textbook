...
\begin{enumerate}
    \item Understand training objectives and loss functions for different transformer architectures
    \item Analyze gradient flow and backpropagation through transformer layers
    \item Implement optimization algorithms (Adam, AdamW, LAMB) with appropriate hyperparameters
    \item Apply learning rate schedules with warmup and decay
    \item Use mixed precision training to reduce memory and accelerate training
    \item Apply gradient accumulation and checkpointing for memory-constrained scenarios
    \item Understand distributed training strategies for large-scale models
    \item Select appropriate batch sizes and sequence lengths based on hardware constraints
    \item Apply regularization techniques to prevent overfitting
    \item Estimate training time and costs for transformer models
\end{enumerate}
...
\section{Backpropagation Through Transformers}
...
\subsection{Gradients Through Residual Connections}
...
\begin{tikzpicture}[
  ...
  arrow/.style={->, thick},
  gradient/.style={->, thick, red, dashed}
]
...
\draw[arrow] (x) -- (F);
\draw[arrow] (F) -- (add);
\draw[arrow, blue, very thick] (x) to[bend left=30] (add);
\draw[arrow] (add) -- (y);

% Backward pass (gradients) - text positioned absolutely at fixed coordinates
...
% Gradient arrows at fixed y-coordinate
\draw[gradient] (8,3) -- (6,3);
\draw[gradient] (6,3) -- (3,3);
\draw[gradient, blue, very thick] (6,3) to[bend right=30] (0,3);
\draw[gradient] (3,3) -- (0,3);

\end{tikzpicture}
\caption{Gradient flow through residual connections. The blue path shows the direct gradient highway that bypasses $F(\mathbf{x})$, ensuring gradients can flow through many layers without vanishing. Red dashed arrows show gradient flow during backpropagation.}
\label{fig:residual_gradient_flow}
\end{figure}
...
\subsection{Gradients Through Layer Normalization}
...
\subsection{Gradients Through Attention}
...
\subsection{Gradients Through Feed-Forward Networks}
...
\subsection{Computational Cost of Backpropagation}
...
\begin{itemize}
    \item Parameters: $P \times 4$ bytes
    \item Gradients: $P \times 4$ bytes
    \item First moments: $P \times 4$ bytes
    \item Second moments: $P \times 4$ bytes
    \item Total: $16P$ bytes
\end{itemize}
...
\subsection{LAMB: Large Batch Training}
...
\subsection{Optimizer Memory Comparison}

Different optimizers have different memory footprints, which can be a critical consideration for large models:

\begin{itemize}
    \item \textbf{SGD (no momentum):} $8P$ bytes (parameters + gradients in FP32)
    \item \textbf{SGD with momentum:} $12P$ bytes (parameters + gradients + momentum in FP32)
    \item \textbf{Adam/AdamW/LAMB:} $16P$ bytes (parameters + gradients + first moment + second moment in FP32)
\end{itemize}

For BERT-base with 110 million parameters:
\begin{itemize}
    \item SGD: $110\text{M} \times 8 = 880$ MB
    \item SGD with momentum: $110\text{M} \times 12 = 1{,}320$ MB
    \item Adam/AdamW/LAMB: $110\text{M} \times 16 = 1{,}760$ MB
\end{itemize}
...
\subsection{The Necessity of Warmup}
...
\subsection{Warmup Plus Linear Decay}
...
\subsection{FP16 Training Algorithm}
...
\begin{enumerate}
    \item \textbf{Forward pass:} Convert FP32 master weights to FP16, perform all forward computations in FP16, producing FP16 activations
    \item \textbf{Loss computation:} Compute loss in FP16, then scale the loss by a large factor $S$ (typically 1024 or dynamically adjusted)
    \item \textbf{Backward pass:} Compute gradients in FP16 using the scaled loss, producing FP16 gradients that are also scaled by $S$
    \item \textbf{Gradient unscaling:} Divide FP16 gradients by $S$ to recover the true gradient scale
    \item \textbf{Gradient conversion:} Convert unscaled FP16 gradients to FP32
    \item \textbf{Parameter update:} Update FP32 master weights using FP32 gradients and the optimizer
    \item \textbf{Repeat:} Copy updated FP32 weights to FP16 for the next iteration
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{FP32 optimizer states (Adam):} $8P$ bytes (first and second moments)
    \item \textbf{FP16 activations:} $A/2$ bytes (where $A$ is FP32 activation memory)
\end{itemize}
...
\begin{itemize}
    \item \textbf{FP32 performance:} 156 TFLOPS (teraflops)
    \item \textbf{FP16 performance (Tensor Cores):} 312 TFLOPS
    \item \textbf{Theoretical speedup:} 2×
\end{itemize}

In practice, the speedup is typically 1.5-1.8× rather than the full 2× because:
\begin{itemize}
    \item Not all operations benefit from FP16 (e.g., layer normalization, softmax, and other element-wise operations may still run in FP32 for numerical stability)
    \item Memory bandwidth limitations can bottleneck performance, particularly for small batch sizes
    \item Overhead from data type conversions and loss scaling
    \item Non-matrix operations (activations, normalizations) don't use Tensor Cores
\end{itemize}
...
\subsection{BF16: An Alternative to FP16}
...
\begin{enumerate}
    ...
    \item Loss computation in BF16 (no scaling needed)
    \item Backward pass in BF16
    \item Convert BF16 gradients to FP32
    \item Update FP32 master weights
\end{enumerate}
...
\label{sec:gradient_accumulation}
...
\label{alg:gradient_accumulation_detailed}
\textbf{Input:} Mini-batch size $B_{\text{mini}}$, accumulation steps $K$, dataset

\begin{algorithmic}[1]
\State Initialize model parameters $\mathbf{w}$
\State Initialize optimizer
\For{each epoch}
    \For{each batch of $K$ mini-batches}
        \State Zero accumulated gradients: $\mathbf{g}_{\text{accum}} = \mathbf{0}$
        \For{$k = 1$ to $K$}
            \State Load mini-batch $\mathcal{B}_k$ of size $B_{\text{mini}}$
            \State Forward pass: compute loss $L_k$ on $\mathcal{B}_k$
            \State Scale loss: $L_k \leftarrow L_k / K$
            \State Backward pass: compute gradients $\mathbf{g}_k$
            \State Accumulate: $\mathbf{g}_{\text{accum}} \leftarrow \mathbf{g}_{\text{accum}} + \mathbf{g}_k$
        \EndFor
        \State Update parameters using $\mathbf{g}_{\text{accum}}$
        \State Zero gradients for next accumulation
    \EndFor
\EndFor
\end{algorithmic}
\end{algorithm}
...
In PyTorch, gradient accumulation is implemented by simply not calling \texttt{optimizer.zero\_grad()} after each mini-batch. Gradients accumulate automatically because PyTorch adds new gradients to existing gradients by default:
...
\textbf{Memory:} Gradient accumulation requires only the memory for a single mini-batch of size $B_{\text{mini}}$, not the full effective batch size $B_{\text{eff}}$. This is the primary benefit—it enables training with large effective batch sizes on memory-constrained hardware.

\textbf{Computation time:} Gradient accumulation is slower than true large-batch training because the mini-batches are processed sequentially rather than in parallel. For $K$ accumulation steps, we perform $K$ forward passes and $K$ backward passes before a single parameter update. If we could fit the full batch in memory, we would perform 1 forward pass and 1 backward pass, processing $K$ times more data in parallel.

The time overhead is typically 10-20\% compared to true large-batch training, arising from:
\begin{itemize}
    \item Reduced parallelism: processing mini-batches sequentially rather than in parallel
    \item Increased overhead: $K$ forward-backward passes have more overhead than 1 pass
    \item Memory bandwidth: loading model parameters $K$ times rather than once
\end{itemize}

\textbf{Batch normalization incompatibility:} Gradient accumulation is incompatible with batch normalization because batch normalization computes statistics over the mini-batch, not the effective batch. Each mini-batch has different statistics, leading to incorrect normalization. Fortunately, transformers use layer normalization rather than batch normalization, so this is not a concern for transformer training.
...
\begin{itemize}
    \item Without accumulation (batch 512): $\approx 220$ GB (exceeds any single GPU)
    \item With accumulation (batch 32): $\approx 13.8$ GB (fits on V100 16GB)
\end{itemize}

\textbf{Training time comparison:}
\begin{itemize}
    \item True batch 512 (if it fit): 1 forward + 1 backward = 2 passes
    \item Gradient accumulation: 16 forward + 16 backward = 32 passes
\end{itemize}
...
\textbf{When to use gradient accumulation:}
\begin{itemize}
    \item When the desired batch size exceeds GPU memory capacity
    \item When trying to match published training recipes that use large batches
    \item When larger batches improve convergence (common for transformers)
    \item When training time is less critical than achieving good final performance
\end{itemize}

\textbf{When not to use gradient accumulation:}
\begin{itemize}
    \item When the mini-batch size is already optimal for convergence
    \item When training time is critical and larger batches don't improve convergence
    \item When the overhead (15-20\%) is unacceptable
\end{itemize}
...
\subsection{The Memory-Computation Trade-off}
...
In PyTorch, gradient checkpointing is implemented using \texttt{torch.utils.checkpoint.checkpoint}, which wraps a function and handles the recomputation automatically:
...
During the forward pass, PyTorch executes \texttt{\_forward} but doesn't store intermediate activations. During the backward pass, when gradients reach this layer, PyTorch re-executes \texttt{\_forward} with the saved input \texttt{x}, reconstructing the intermediate activations needed for gradient computation.
...
\textbf{Training time impact:}
\begin{itemize}
    \item Without checkpointing: 100\% (baseline)
    \item With checkpointing: 125\% (25\% slower)
\end{itemize}
...
\subsection{When to Use Gradient Checkpointing}

Gradient checkpointing is most beneficial in specific scenarios:

\textbf{Use checkpointing when:}
\begin{itemize}
    \item Training with long sequences (e.g., $n > 1024$) where activation memory dominates
    \item GPU memory is the limiting factor preventing larger batch sizes
    \item The model is very deep (many layers) and activation memory scales linearly with depth
    \item Training time is less critical than maximizing batch size or sequence length
    \item Combined with mixed precision, checkpointing enables training that would otherwise be impossible
\end{itemize}

\textbf{Avoid checkpointing when:}
\begin{itemize}
    \item Memory is not constrained and the 20-30\% time overhead is unacceptable
    \item Training with short sequences and small batch sizes where activation memory is already manageable
    \item Optimizing for minimum training time rather than maximum throughput
    \item The model is shallow enough that activation memory is not the bottleneck
\end{itemize}
...
\begin{enumerate}
    \item Each GPU has a complete copy of the model
    \item The global batch is split across GPUs: GPU $i$ processes mini-batch $\mathcal{B}_i$
    \item Each GPU performs forward and backward passes independently, computing local gradients $\mathbf{g}_i$
    \item AllReduce operation computes the average gradient: $\bar{\mathbf{g}} = \frac{1}{N}\sum_{i=1}^{N} \mathbf{g}_i$ where $N$ is the number of GPUs
    \item Each GPU updates its model using $\bar{\mathbf{g}}$
    \item All GPUs now have identical models (up to floating-point precision)
\end{enumerate}
...
\begin{itemize}
    \item Time 1: GPU 1 processes micro-batch 1 (forward)
    \item Time 2: GPU 1 processes micro-batch 2 (forward), GPU 2 processes micro-batch 1 (forward)
    \item Time 3: GPU 1 processes micro-batch 3 (forward), GPU 2 processes micro-batch 2 (forward), GPU 3 processes micro-batch 1 (forward)
    \item Time 4: All GPUs are active, processing different micro-batches
\end{itemize}
...
\begin{itemize}
    \item Without ZeRO: Each GPU stores 880 MB of optimizer states
    \item With ZeRO-1: Each GPU stores $880 / 8 = 110$ MB of optimizer states
    \item Memory saved: 770 MB per GPU
\end{itemize}
...
\begin{itemize}
    \item Without ZeRO: Each GPU stores 440 MB of gradients
    \item With ZeRO-2: Each GPU stores $440 / 8 = 55$ MB of gradients
    \item Total memory saved: $770 + 385 = 1{,}155$ MB per GPU
\end{itemize}
...
For BERT-base with 8 GPUs:
\begin{itemize}
    \item Without ZeRO: Each GPU stores 440 MB of parameters
    \item With ZeRO-3: Each GPU stores $440 / 8 = 55$ MB of parameters
    \item Total memory saved: $770 + 385 + 385 = 1{,}540$ MB per GPU
\end{itemize}
...
\begin{tabular}{lccl}
\toprule
\textbf{Strategy} & \textbf{Memory Reduction} & \textbf{Communication} & \textbf{Use Case} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Comparison of distributed training strategies for $N$ GPUs}
\end{table}
...
\section{Batch Size and Sequence Length Selection}
\label{sec:batch_sequence_selection}
...
\subsection{Batch Size Considerations}
...
\begin{itemize}
    \item Batch size 8: $\approx 15{,}000$ tokens/sec (30\% GPU utilization)
    \item Batch size 32: $\approx 50{,}000$ tokens/sec (80\% GPU utilization)
    \item Batch size 64: $\approx 70{,}000$ tokens/sec (90\% GPU utilization)
    \item Batch size 128: $\approx 75{,}000$ tokens/sec (95\% GPU utilization)
\end{itemize}
...
\begin{itemize}
    \item Scaling the learning rate proportionally with batch size (linear scaling rule)
    \item Extending the warmup period for larger batches
    \item Training for more steps to compensate for fewer parameter updates
\end{itemize}
...
\subsection{Memory Scaling with Batch Size}
...
\begin{itemize}
    \item Parameters + optimizer: 1.76 GB (independent of batch size)
    \item Activations: 12 GB (scales linearly with batch size)
\end{itemize}
...
\subsection{Sequence Length Considerations}
...
\begin{itemize}
    \item Efficient attention mechanisms (sparse attention, linear attention) that reduce the $O(n^2)$ complexity
    \item Gradient checkpointing to reduce activation memory
    \item Smaller batch sizes to fit within memory constraints
    \item More powerful GPUs with larger memory
\end{itemize}
...
\begin{itemize}
    \item Without dynamic batching: 50,000 tokens/sec (including padding)
    \item With dynamic batching: 70,000 tokens/sec (40\% improvement)
\end{itemize}
...
\textbf{For batch size:}
\begin{itemize}
    \item Start with the largest batch size that fits in GPU memory
    \item If memory-constrained, use gradient accumulation to achieve larger effective batch sizes
    \item For BERT-base on V100 (16 GB): batch size 16-32 with sequence length 512
    \item For BERT-base on A100 (40 GB): batch size 32-64 with sequence length 512
    \item Scale learning rate proportionally when increasing batch size
    \item Extend warmup period for very large batches (>1024)
\end{itemize}

\textbf{For sequence length:}
\begin{itemize}
    \item Use the longest sequence length that fits in memory and is relevant for the task
    \item For memory-constrained scenarios, reduce batch size rather than sequence length if long context is important
    \item Use gradient checkpointing to enable longer sequences
    \item Consider efficient attention mechanisms for sequences longer than 2048
    \item Use dynamic batching to reduce padding waste
\end{itemize}

\textbf{Memory-constrained optimization:}
\begin{enumerate}
    \item Enable mixed precision training (FP16/BF16): 40-50\% memory reduction
    \item Enable gradient checkpointing: 80\% activation memory reduction
    \item Use gradient accumulation: simulate large batches with small physical batches
    \item Reduce sequence length if task permits
    \item Use dynamic batching to reduce padding waste
\end{enumerate}
...
\textbf{Attention dropout:} Applied to the attention weights after softmax, before multiplying by values:
\begin{equation}
...
\textbf{Residual dropout:} Applied to the output of each sub-layer before adding to the residual connection:
\begin{equation}
...
\textbf{Embedding dropout:} Applied to the sum of token embeddings and positional encodings:
\begin{equation}
...
\section{Training Time and Cost Estimates}
\label{sec:training_costs}
...
\begin{itemize}
    \item Hardware: 16× NVIDIA V100 GPUs (16 GB each)
    \item Batch size: 256 per GPU, 4096 total effective batch size
    \item Sequence length: 512 tokens
    \item Training data: 3.3 billion words (approximately 16 GB of text)
    \item Training steps: 1 million steps
    \item Optimizer: AdamW with learning rate $1 \times 10^{-4}$, warmup 10,000 steps
\end{itemize}
...
\textbf{Training configuration (GPT-2 XL):}
\begin{itemize}
    \item Parameters: 1.5 billion
    \item Hardware: 32× NVIDIA V100 GPUs
    \item Training data: 40 GB of text (WebText dataset)
    \item Sequence length: 1024 tokens
    \item Batch size: 512 total effective batch size
    \item Training time: Approximately 1 week
\end{itemize}
...
\begin{itemize}
    ...
    \item Architecture: 96 layers, $d_{\text{model}} = 12{,}288$, $d_{ff} = 49{,}152$
    \item Training data: 300 billion tokens (approximately 570 GB of text)
    \item Sequence length: 2048 tokens
    \item Hardware: Estimated 10,000+ NVIDIA V100 GPUs (or equivalent)
    \item Training time: Approximately 1 month
\end{itemize}
...
\begin{itemize}
    \item Compute cost: \$4-12 million (depending on hardware and efficiency)
    \item Energy consumption: Approximately 1,287 MWh
    \item Carbon footprint: Approximately 552 metric tons CO$_2$ equivalent (depending on energy source)
\end{itemize}
...
\textbf{Key scaling relationships:}

\textbf{Compute scaling:} Doubling the model size (number of parameters) requires approximately 4× the compute for the same amount of training data. This quadratic scaling arises because:
\begin{itemize}
    \item FLOPs scale linearly with parameters: $\text{FLOPs} \propto P$
    \item Optimal training data scales linearly with parameters: $\text{Data} \propto P$
    \item Total compute is FLOPs × Data: $\text{Compute} \propto P^2$
\end{itemize}

\textbf{Data scaling:} Doubling the dataset size requires approximately 2× the compute (assuming model size is fixed). This linear scaling is straightforward: processing twice as much data requires twice as many training steps.

\textbf{Optimal allocation:} For a fixed compute budget $C$, the optimal allocation between model size $P$ and dataset size $D$ follows:
\begin{equation}
P \propto C^{0.73}, \quad D \propto C^{0.27}
\end{equation}

This means that as compute increases, most of the additional compute should go toward larger models rather than more data. For example, increasing compute by 10× should increase model size by approximately 5.4× and dataset size by approximately 1.9×.
...
\begin{itemize}
    \item BERT-base (110M params): \$7,000, strong performance on many tasks
    \item BERT-large (340M params): \$25,000, 2-3\% improvement over BERT-base
    \item GPT-2 XL (1.5B params): \$50,000, significant improvement in generation quality
    \item GPT-3 (175B params): \$4-12 million, state-of-the-art few-shot learning
\end{itemize}
...
\textbf{Tokenization:} Use subword tokenization (BPE, WordPiece, or SentencePiece) to balance vocabulary size and representation quality. For English, a vocabulary size of 30,000-50,000 works well. Train the tokenizer on a representative sample of your data (at least 1 million sentences) to ensure good coverage. The tokenizer should handle rare words, numbers, and special characters appropriately.

\textbf{Sequence packing:} Combine multiple short documents into single sequences to minimize padding waste. For example, if training with sequence length 512, pack documents separated by special tokens until reaching 512 tokens. This improves efficiency by ensuring most tokens in each sequence are meaningful rather than padding.

\textbf{Data augmentation:} For tasks where data is limited, consider augmentation strategies like back-translation, synonym replacement, or random insertion/deletion of tokens. However, for large-scale pretraining, augmentation is typically unnecessary and may hurt performance by introducing noise.

\textbf{Data filtering:} Remove low-quality examples (duplicates, non-linguistic content, extremely short or long sequences) to improve training efficiency. For web-scraped data, filter by language, remove boilerplate content, and deduplicate at the document level.
...
\textbf{Weight initialization:} Use Xavier (Glorot) initialization for linear layers:
\begin{equation}
W_{ij} \sim \mathcal{N}(0, \frac{2}{d_{\text{in}} + d_{\text{out}}})
\end{equation}
where $d_{\text{in}}$ and $d_{\text{out}}$ are the input and output dimensions. This initialization maintains variance across layers, preventing vanishing or exploding activations.

\textbf{Bias initialization:} Initialize all biases to zero. This is standard practice and works well for transformers.

\textbf{Embedding initialization:} Initialize token embeddings with Xavier initialization. Position embeddings can be initialized randomly or with sinusoidal patterns (as in the original Transformer). Random initialization is more common in modern models and allows the model to learn task-specific positional patterns.

\textbf{Layer normalization initialization:} Initialize scale parameters $\gamma$ to 1 and shift parameters $\beta$ to 0. This makes layer normalization initially act as the identity function, allowing gradients to flow freely in early training.

\textbf{Output layer initialization:} For the output projection to vocabulary logits, use Xavier initialization with a smaller scale (multiply by 0.5 or 0.1) to prevent overly confident initial predictions that can destabilize training.
...
\textbf{Learning rate:} Start with $\eta_{\max} = 1 \times 10^{-4}$ to $3 \times 10^{-4}$ for Adam/AdamW. Larger models typically require smaller learning rates. For BERT-base, use $1 \times 10^{-4}$. For GPT-2, use $2.5 \times 10^{-4}$. For models larger than 1B parameters, use $6 \times 10^{-5}$ or smaller.

\textbf{Warmup:} Use 10\% of total training steps for warmup, or at least 1,000 steps. For very large models or large batch sizes, extend warmup to 20\% of steps. The warmup period should be long enough for Adam's moment estimates to stabilize.

\textbf{Batch size:} Use the largest batch size that fits in memory, typically 256-2048 for transformers. If memory-constrained, use gradient accumulation to achieve larger effective batch sizes. Scale the learning rate proportionally when increasing batch size beyond 256.

\textbf{Weight decay:} Use $\lambda = 0.01$ for AdamW. Exclude biases and layer normalization parameters from weight decay. This moderate regularization prevents overfitting without overly constraining the model.

\textbf{Dropout:} Use $p = 0.1$ for all dropout locations (attention, residual, embedding). For small datasets, increase to $p = 0.2$ or $p = 0.3$. For very large models (>10B parameters), consider reducing to $p = 0.05$.

\textbf{Gradient clipping:} Use threshold $\theta = 1.0$. Monitor the fraction of steps where clipping occurs (should be 1-5\%). If clipping occurs more frequently, consider reducing the learning rate.

\textbf{Adam hyperparameters:} Use $\beta_1 = 0.9$, $\beta_2 = 0.999$, $\epsilon = 10^{-8}$. These values work well across different model sizes and tasks.
...
\textbf{Mixed precision:} Enable FP16 or BF16 mixed precision training to reduce memory and accelerate computation. Use automatic mixed precision (AMP) libraries like PyTorch's \texttt{torch.cuda.amp} or NVIDIA Apex to handle the complexity of loss scaling and data type conversions.

\textbf{Gradient accumulation:} If using gradient accumulation, ensure the loss is scaled by $1/K$ where $K$ is the number of accumulation steps. This ensures the accumulated gradient has the correct magnitude.

\textbf{Gradient checkpointing:} Enable gradient checkpointing if memory-constrained, particularly for long sequences or deep models. The 20-30\% time overhead is usually acceptable given the memory savings.

\textbf{Learning rate schedule:} Implement the chosen schedule (warmup + linear decay, warmup + cosine, etc.) and update the learning rate at each step. Most optimization libraries provide schedulers that handle this automatically.

\textbf{Validation:} Evaluate on a validation set every $N$ steps (typically every 1,000-10,000 steps depending on dataset size). Compute validation loss and task-specific metrics. Use validation performance to detect overfitting and select the best checkpoint.

\textbf{Checkpointing:} Save model checkpoints regularly (every 10,000-50,000 steps) to enable recovery from failures and to preserve the best model. Save optimizer state along with model parameters to enable seamless resumption of training.
...
\textbf{Training loss:} Should decrease steadily. If loss plateaus early, the learning rate may be too low. If loss spikes or diverges, the learning rate may be too high or gradient clipping may be insufficient.

\textbf{Validation loss:} Should track training loss initially, then diverge as the model begins to overfit. If validation loss increases while training loss decreases, increase regularization (dropout, weight decay) or reduce model capacity.

\textbf{Perplexity:} For language modeling, perplexity $= \exp(\text{loss})$ provides an interpretable metric. Lower perplexity indicates better predictions. BERT-base achieves perplexity around 3-4 on masked language modeling.

\textbf{Learning rate:} Monitor the current learning rate to verify the schedule is working correctly. The learning rate should increase during warmup, then decrease during decay.

\textbf{Gradient norm:} Track the norm of the gradient vector. Typical values are 0.1-10. Very small gradients (<0.01) may indicate vanishing gradients or a learning rate that's too low. Very large gradients (>100) may indicate exploding gradients or a learning rate that's too high.

\textbf{Parameter norm:} Track the norm of the parameter vector. This should increase gradually during training as the model learns. Sudden jumps may indicate instability.

\textbf{GPU memory usage:} Monitor memory consumption to ensure you're using available memory efficiently. If memory usage is much lower than GPU capacity, consider increasing batch size.

\textbf{Throughput:} Track tokens processed per second. This helps identify performance bottlenecks and measure the impact of optimizations.

\textbf{Common issues and solutions:}

\textit{Loss doesn't decrease:} Check learning rate (may be too low), verify data is loading correctly, check initialization (may be poor), ensure gradients are flowing (check gradient norms).

\textit{Loss spikes or diverges:} Reduce learning rate, increase warmup period, enable or strengthen gradient clipping, check for data quality issues (corrupted examples, extreme outliers).

\textit{Training is slow:} Enable mixed precision if not already enabled, increase batch size if memory allows, use gradient accumulation to increase effective batch size, profile to identify bottlenecks (data loading, computation, communication).

\textit{Out of memory:} Reduce batch size, enable gradient checkpointing, reduce sequence length, enable mixed precision, use gradient accumulation to maintain effective batch size.

\textit{Poor generalization:} Increase regularization (dropout, weight decay), use label smoothing, train on more data, reduce model capacity, use data augmentation.
...
\begin{exercise}
Implement the complete mixed precision training algorithm for a small transformer. Compare memory consumption and training time with FP32 training. Experiment with different loss scaling factors and observe their impact on training stability.
\end{exercise}

\begin{exercise}
For BERT-base with batch size 32 and sequence length 512, calculate the exact memory requirements for: (a) parameters and optimizer states (AdamW), (b) activations for each layer type, (c) total memory with and without gradient checkpointing. Verify your calculations by profiling actual memory usage during training.
\end{exercise}

\begin{exercise}
Implement gradient accumulation to achieve an effective batch size of 512 with physical batch size 32. Measure the training time overhead compared to true batch size 512 (if it fits in memory). Verify that the training dynamics are identical by comparing loss curves.
\end{exercise}

\begin{exercise}
Train a small transformer (6 layers, $d_{\text{model}} = 256$) with different learning rate schedules: (a) warmup + linear decay, (b) warmup + inverse square root decay, (c) warmup + cosine annealing. Compare convergence speed and final performance. Plot the learning rate curves and loss curves.
\end{exercise}

\begin{exercise}
Implement data parallelism for training on 4 GPUs. Measure the speedup compared to single-GPU training. Calculate the communication overhead by comparing the time spent in AllReduce operations versus computation. Experiment with different batch sizes and observe how they affect the computation-to-communication ratio.
\end{exercise}

\begin{exercise}
Analyze the impact of different regularization techniques on a small transformer trained on a limited dataset (10,000 examples). Compare: (a) no regularization, (b) dropout only, (c) weight decay only, (d) dropout + weight decay, (e) dropout + weight decay + label smoothing. Measure training loss, validation loss, and generalization gap.
\end{exercise}

\begin{exercise}
Estimate the training time and cost for a GPT-2 medium model (345M parameters) on your available hardware. Calculate: (a) FLOPs per training step, (b) expected throughput (tokens/sec), (c) total training time for 10B tokens, (d) estimated cost on cloud platforms. Compare your estimates with actual training runs.
\end{exercise}

\begin{exercise}
Implement dynamic batching to minimize padding waste. Compare throughput (tokens/sec) with and without dynamic batching on a dataset with variable-length sequences. Measure the padding fraction in each case and calculate the theoretical maximum speedup from eliminating padding.
\end{exercise}
...
\begin{solution}
\textbf{Exercise 1: Mixed Precision Training Implementation}
...
\textbf{Loss Scaling Impact:}

\begin{itemize}
    \item \textbf{Too low ($2^8$):} Gradient underflow, training instability
    \item \textbf{Optimal ($2^{16}$):} Stable training, good convergence
    \item \textbf{Too high ($2^{24}$):} Gradient overflow, NaN losses
\end{itemize}

\textbf{Key Observations:}
\begin{enumerate}
    \item Memory reduction: $\sim$45\% (activations stored in FP16)
    \item Speed improvement: $\sim$57\% (faster tensor core operations)
    \item Dynamic loss scaling automatically adjusts to prevent overflow/underflow
    \item No accuracy degradation with proper loss scaling
\end{enumerate}
\end{solution}


\begin{solution}
\textbf{Exercise 2: BERT-base Memory Calculation}

Given: BERT-base with batch size $B=32$, sequence length $L=512$, $d_{\text{model}}=768$, $N=12$ layers, $h=12$ heads, $d_{ff}=3072$

\textbf{Part (a): Parameters and Optimizer States}

\textbf{Model Parameters:}
\begin{itemize}
    \item Embeddings: $V \times d_{\text{model}} = 30{,}000 \times 768 = 23{,}040{,}000$
    \item Position embeddings: $512 \times 768 = 393{,}216$
    \item Per encoder layer:
    \begin{itemize}
        \item Attention: $4 \times 768^2 = 2{,}359{,}296$
        \item FFN: $768 \times 3072 + 3072 \times 768 = 4{,}718{,}592$
        \item LayerNorm: $2 \times 2 \times 768 = 3{,}072$
        \item Total per layer: $7{,}080{,}960$
    \end{itemize}
    \item 12 layers: $12 \times 7{,}080{,}960 = 84{,}971{,}520$
    \item Pooler: $768 \times 768 = 589{,}824$
    \item \textbf{Total parameters: $109{,}994{,}560 \approx 110$M}
\end{itemize}

Memory for parameters (FP32): $110M \times 4 \text{ bytes} = 440$MB

\textbf{AdamW Optimizer States:}
\begin{itemize}
    \item First moment (momentum): $110M \times 4 = 440$MB
    \item Second moment (variance): $110M \times 4 = 440$MB
    \item \textbf{Total optimizer: $880$MB}
\end{itemize}

\textbf{Total for parameters + optimizer: $440 + 880 = 1{,}320$MB}


\textbf{Part (b): Activations per Layer Type}
...
\textbf{Per Encoder Layer:}
\begin{itemize}
    \item \textbf{Attention scores:} $B \times h \times L \times L = 32 \times 12 \times 512 \times 512 = 100{,}663{,}296$ floats $= 402.7$MB
    \item \textbf{Attention output:} $B \times L \times d_{\text{model}} = 12{,}582{,}912$ floats $= 50.3$MB
    \item \textbf{FFN intermediate:} $B \times L \times d_{ff} = 32 \times 512 \times 3072 = 50{,}331{,}648$ floats $= 201.3$MB
    \item \textbf{Residual connections:} $2 \times 50.3 = 100.6$MB
    \item \textbf{Total per layer: $754.9$MB}
\end{itemize}

\textbf{All 12 layers: $12 \times 754.9 = 9{,}058.8$MB}

\textbf{Gradients:} Same size as activations $= 9{,}058.8$MB

\textbf{Total activations + gradients: $18{,}117.6$MB $\approx 18.1$GB}


\textbf{Part (c): Total Memory With/Without Gradient Checkpointing}

\textbf{Without Gradient Checkpointing:}
\begin{itemize}
    \item Parameters: $440$MB
    \item Optimizer states: $880$MB
    \item Activations: $9{,}059$MB
    \item Gradients: $9{,}059$MB
    \item \textbf{Total: $19{,}438$MB $\approx 19.4$GB}
\end{itemize}

\textbf{With Gradient Checkpointing:}

Store only activations at checkpoints (every 2 layers), recompute others during backward:
\begin{itemize}
    \item Checkpointed activations: $6 \times 754.9 = 4{,}529$MB (6 checkpoints)
    \item Recomputed during backward: $6 \times 754.9 = 4{,}529$MB (not stored)
    \item Gradients: $9{,}059$MB (same)
    \item \textbf{Activation memory: $4{,}529$MB (50\% reduction)}
\end{itemize}

\textbf{Total with checkpointing: $440 + 880 + 4{,}529 + 9{,}059 = 14{,}908$MB $\approx 14.9$GB}

\textbf{Memory savings: $19.4 - 14.9 = 4.5$GB (23\% reduction)}

\textbf{Trade-off:} 33\% increase in computation time (recomputing 6 layers during backward)

\textbf{Verification with PyTorch Profiler:}
\begin{lstlisting}[language=Python]
import torch
from torch.utils.checkpoint import checkpoint
...
\begin{solution}
\textbf{Exercise 3: Gradient Accumulation Implementation}
...
\textbf{Time Overhead Analysis:}

The 20.8\% overhead comes from:
\begin{enumerate}
    \item \textbf{Multiple forward passes:} 16 forward passes vs 1 (but each is smaller)
    \item \textbf{Memory transfers:} More frequent CPU-GPU data transfers
    \item \textbf{Kernel launch overhead:} 16$\times$ more kernel launches
    \item \textbf{No parallelism across accumulation steps:} Sequential execution
\end{enumerate}

\textbf{Loss Curve Verification:}
...
\textbf{Key Findings:}
\begin{itemize}
    \item Loss curves are nearly identical (correlation $>$ 0.99)
    \item Training dynamics match exactly (same effective batch size)
    \item Memory usage reduced by 77\% (enables training on smaller GPUs)
    \item Time overhead is acceptable for memory-constrained scenarios
\end{itemize}
\end{solution}


\begin{solution}
\textbf{Exercise 4: Learning Rate Schedule Comparison}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Aggressive decay can hurt final performance
        \item Works well when total training steps known in advance
    \end{itemize}
    
    \item \textbf{Inverse Square Root:}
    \begin{itemize}
        \item Used in original Transformer paper
        \item Slower decay allows continued learning
        \item Better for open-ended training
        \item Formula: $\text{lr} = \frac{1}{\sqrt{\max(step, warmup)}}$
    \end{itemize}
    
    \item \textbf{Cosine Annealing:}
    \begin{itemize}
        \item Smooth decay with gradual slowdown
        \item Best final performance in experiments
        \item Allows fine-tuning near convergence
        \item Popular in modern transformer training
    \end{itemize}
\end{enumerate}

\textbf{Warmup Importance:}

All schedules use warmup (4000 steps) to:
\begin{itemize}
    \item Prevent early training instability
    \item Allow optimizer statistics to stabilize
    \item Avoid large gradient updates with random initialization
\end{itemize}

\textbf{Recommendation:} Cosine annealing with warmup provides best balance of convergence speed and final performance for most transformer training scenarios.
\end{solution}


\begin{solution}
\textbf{Exercise 5: Data Parallelism Implementation}
...
\begin{itemize}
    \item 2 GPUs: $\rho = 15/120 = 12.5\%$
    \item 4 GPUs: $\rho = 18/60 = 30\%$
    \item 8 GPUs: $\rho = 22/30 = 73\%$
\end{itemize}

As GPU count increases, communication becomes bottleneck.

\textbf{Batch Size Impact:}
...
\begin{itemize}
    \item Computation scales with batch size
    \item Communication (gradient size) is independent of batch size
    \item Better GPU utilization with larger batches
\end{itemize}

\textbf{Optimal Configuration:} 4 GPUs with batch size 32-64 per GPU provides best balance of speedup and efficiency.
\end{solution}


\begin{solution}
\textbf{Exercise 6: Regularization Techniques Analysis}
...
\textbf{Experimental Results (10,000 training examples):}
...
\textbf{(a) No Regularization:}
\begin{itemize}
    \item Severe overfitting (gap = 2.42)
    \item Low training loss but poor generalization
    \item Model memorizes training data
\end{itemize}

\textbf{(b) Dropout Only:}
\begin{itemize}
    \item Reduces overfitting significantly
    \item Prevents co-adaptation of neurons
    \item Higher training loss (regularization effect)
    \item Validation improves by 9.2\%
\end{itemize}

\textbf{(c) Weight Decay Only:}
\begin{itemize}
    \item Penalizes large weights: $L = L_{\text{task}} + \lambda \|\theta\|^2$
    \item Less effective than dropout alone
    \item Still substantial overfitting
\end{itemize}

\textbf{(d) Dropout + Weight Decay:}
\begin{itemize}
    \item Complementary effects
    \item Dropout: prevents feature co-adaptation
    \item Weight decay: encourages smaller weights
    \item Best combination for standard regularization
\end{itemize}

\textbf{(e) All Three (Dropout + WD + Label Smoothing):}
\begin{itemize}
    \item Label smoothing: $y_{\text{smooth}} = (1-\alpha)y + \alpha/K$
    \item Prevents overconfident predictions
    \item Smallest generalization gap (0.91)
    \item Best validation accuracy (77.8\%)
    \item Recommended for limited data scenarios
\end{itemize}

\textbf{Generalization Gap:} $\text{Gap} = L_{\text{val}} - L_{\text{train}}$
...
\begin{solution}
\textbf{Exercise 7: GPT-2 Medium Training Estimation}

Given: GPT-2 Medium with 345M parameters, training on 10B tokens

\textbf{Part (a): FLOPs per Training Step}
...
\textbf{Part (b): Expected Throughput}
...
\textbf{Part (c): Total Training Time}
...
\textbf{Part (d): Cloud Cost Estimation}
...
\begin{itemize}
    \item Compute: \$122-147
    \item Storage (checkpoints): \$5-10
    \item Data transfer: \$2-5
    \item \textbf{Total estimated cost: \$130-160}
\end{itemize}

\textbf{Comparison with Actual Runs:}
...
\begin{solution}
\textbf{Exercise 8: Dynamic Batching Implementation}
...
\begin{itemize}
    \item Remaining padding (8.2\%)
    \item Variable batch sizes (less efficient GPU utilization)
    \item Sorting overhead
\end{itemize}

\textbf{Key Insights:}
\begin{enumerate}
    \item Dynamic batching dramatically reduces wasted computation
    \item Most effective for datasets with high length variance
    \item Trade-off: slightly more complex data loading
    \item Essential for efficient training on real-world data
\end{enumerate}
\end{solution}
